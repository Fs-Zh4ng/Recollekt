...
function extractKeyFromUrl(url) {
  try {
    ...
    return decodeURIComponent(urlParts.pathname.substring(1)); // Remove leading '/'
  } catch (error) {
...
// Connect to MongoDB
mongoose.connect('mongodb+srv://ferdinandsz08:OJk78J5HhbxjukZj@cluster0.zroqlyi.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0', {
...
const getBase64FromS3 = async (bucketName, key) => {
  ...
  try {
    ...
    console.log("Base 64", base64.substring(0, 100)); // Debugging log to check the first 100 characters of the Base64 string
...
app.get('/albums/:id/images', async (req, res) => {
  ...
  try {
    ...
    const user = await User.findById(userId).populate('sharedAlbums'); // Assuming `sharedAlbums` is populated

    if (!user) {
      return res.status(404).json({ message: 'User not found' });
    }

    // Check if the album exists in the user's albums or shared albums
    const album = await Album.findOne({
      _id: req.params.id,
      $or: [
        { creatorId: userId }, // Album owned by the user
        { _id: { $in: user.sharedAlbums.map((sharedAlbum) => sharedAlbum._id) } }, // Album in shared albums
      ],
    });
...
app.get('/user/profile', async (req, res) => {
  ...
  try {
    ...
    const b64 = await getBase64FromS3(
      process.env.AWS_S3_BUCKET_NAME,
      extractKeyFromUrl(user.profileImage)
    );
    user.profileImage = b64; // Convert profile image URL to Base64
    console.log('User profile image:', user.profileImage.substring(0, 100)); // Debugging log
...
app.post('/generate-video', upload.none(), async (req, res) => {
  try {
    ...
    if (!albums || !Array.isArray(albums) || albums.length === 0) {
      return res.status(400).json({ error: 'No valid albums provided' });
    }
    ...
    const videoUrl = `http://35.183.184.126:3000:${PORT}/videos/${path.basename(outputVideoPath)}`;
...
// Start the server
const HOSTNAME = '0.0.0.0'; // Replace with your desired hostname
app.listen(PORT, HOSTNAME, () => {
  console.log(`Server is running on http://${HOSTNAME}:80`);
});